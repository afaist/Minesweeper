...
0.036410034
0.9411751338804492
0.2533641025032649</code></pre> 
  <p>Notice that if you run it again, the number will be different, as you can see with the last two numbers. Even though we called the same <code class="language-kotlin">nextDouble()</code> function, the numbers are not the same. That is because the generator gives us a new number every time: they could repeat sometimes but the probability of that is very low.</p> 
  <h5>Custom ranges</h5> 
  <p>As you can see, some numbers ended up pretty big, others small, and one is even negative. The default ranges for integers and floating-point numbers are different and chosen to cover the most common cases. But what if we needed only the integers between 0 and 100 or floats between 0.0 and 5.0? Then, we can specify them explicitly:</p> 
...
Random.nextDouble(0.0, 5.0)</code></pre> 
  <p>Now we can be sure there will be no negatives numbers, all integers will be less than 100, and some floats are greater than 1.0:</p> 
  <pre><code class="language-kotlin">36
 41
 12
 53
 0.00709856968715783
 2.8675389664207414</code></pre> 
  ...
  <h5>Pseudorandom numbers and seeding</h5> 
...
for (i in 0..5) {
  ...
  <p>In Kotlin 1.4, this code will always generate the same five numbers — <code class="language-kotlin">33, 40, 41, 2, 41, 32</code> — even on different machines. In contrast, the default generator will give us a new sequence every time.</p> 
